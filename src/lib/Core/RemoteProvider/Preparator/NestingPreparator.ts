import type IPreparator from './IPreparator'

const REP_PREFIX = 'n'
const REP_PATTERN = new RegExp(`{{${REP_PREFIX}-(\d+)}}`, 'g')

export default class NestingPreparator implements IPreparator {
  private _nestingParams: number[] = []

  constructor(private _itpPattern: RegExp) {}

  prepareBefore(text: string): string {
    text = text.replace(
      this._itpPattern,
      (rpl) => `{{${REP_PREFIX}-${this._nestingParams.push(+rpl) - 1}}}`
    )

    return text
  }

  prepareAfter(text: string): string {
    text = text.replace(REP_PATTERN, (_: unknown, index: string) => {
      return String(this._nestingParams.splice(this._nestingParams[+index] as number, 1))
    })

    return text
  }
}
